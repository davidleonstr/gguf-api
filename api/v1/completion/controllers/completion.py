...
from api.v1.completion.schemas import CompletionRequest, CompletionResponse
...
class ChatCompletionController:
    ...
    def completions(self):
        try:
            request = CompletionRequest(**self.data)
        ...
        try:
            # llm: Llama
            completion = llm.create_completion(**request.model_dump())

            if request.stream:
                return self._handle_stream(completion)

            return Response(data=CompletionResponse(**completion)).send(200)
